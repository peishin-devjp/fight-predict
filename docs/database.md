...
獲得ポイントは試合ごとに小数第1位までとし、小数第2位を四捨五入する。

大会獲得ポイントは、各試合で端数処理した獲得ポイントの合計とする。
...
-   100pt未満でも保存可能
-   100ptを必ず使い切る必要はない
-   未使用ポイントは獲得ポイントへ加算しない
-   100pt超過は保存不可
-   全試合へのポイント配分は必須ではない

ポイント制約はFrontendとBackendの両方で確認する。

Frontendでは、1試合50pt超過および大会合計100pt超過についてユーザーへ即時フィードバックし、不要なPOSTを防止する。

Backendでは、APIを直接呼ばれた場合でも1試合50pt以内・同一大会100pt以内を保証し、不正データがDBへ保存されないようデータ整合性を担保する。
...
難易度倍率はユーザー全体の予想人数比率を主な材料とする。

配分ポイントは各ユーザー自身の自信度を表すものとし、難易度倍率の算出には使用しない。

サービス初期はユーザー予想データが少ないため、外部情報による補助を検討する。

オッズは大会開始時点で確定させる想定。

具体的な倍率テーブル・補間方法・端数処理は`odds.md`で管理する。
...
5.  試合ごとに獲得ポイントを小数第1位へ四捨五入
6.  対象大会の結果を集計
7.  直近3大会の獲得ポイントを集計してランキングを生成

ランキング自体はDBへ保存せず、必要なデータから都度計算する。

MVPでは的中試合数による追加ボーナスは設けない。
